async function loadGoogleMaps() {
    try {
        // console.log("📡 Fetching API key from backend at http://localhost:3000/get-api-key");

        const response = await fetch("https://map-backend-mi7u.onrender.com/get-api-key");
...
async function findNearbyHospitals() {
    ...
    try {
        const response = await fetch(`https://map-backend-mi7u.onrender.com/api/nearby-hospitals?latitude=${latitude}&longitude=${longitude}`);
        ...
        for (const hospital of hospitals) {
            ...
            try {
                const detailsResponse = await fetch(`https://map-backend-mi7u.onrender.com/api/place-details?place_id=${hospital.place_id}`);
            ...
            // Add row to table
            const row = `<tr onclick="showHospitalDetails('${hospital.name}', '${hospital.types?.includes("hospital") ? "Hospital" : "Clinic"}', '${hospital.types?.join(", ") || "N/A"}', '${contactNumber}', '${address}', ${hospital.geometry.location.lat}, ${hospital.geometry.location.lng})">
                <td>${hospital.name}</td>
                <td>${hospital.types?.includes("hospital") ? "Hospital" : "Clinic"}</td>
                <td>${hospital.types?.join(", ") || "N/A"}</td>
                <td>${contactNumber}</td>
                <td>${address}</td>
                <td>${distance} km</td>
            </tr>`;
...
function showHospitalDetails(name, type, specialization, contact, address, lat, lng) {
...
document.getElementById("hospital-table-body").addEventListener("click", function (event) {
    ...
    if (hospital) {
        ...
        showHospitalDetails(name, type, specialization, contact, address, lat, lng);
    }
});
